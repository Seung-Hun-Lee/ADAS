...
def make_exp_name(args, parser):
    exp_name = '{}_{}'.format(args.source_dataset, args.target_dataset)
    dict_args = vars(args)
...
def save_log(prefix, output_dir, date_str, rank=0):
    ...
    print("Logging :", filename)
    logging.basicConfig(level=logging.INFO, format=fmt, datefmt=date_fmt,
                        filename=filename, filemode='w', force=True)
...
def evaluate_eval(args, net, optimizer, scheduler, val_loss, hist, dump_images, writer, best_miou, epoch=0, dataset_name=None, dataset=None, curr_iter=0, optimizer_at=None, scheduler_at=None, save_pth=True):
    """
    Modified IOU mechanism for on-the-fly IOU calculations ( prevents memory overflow for
    large dataset) Only applies to eval/eval.py
    """
    ...
    if val_loss is not None and hist is not None:
        ...
        if writer:
            ...
            if mean_iu > best_miou[dataset_name]:
                best_miou[dataset_name] = mean_iu
                writer.add_scalar('{}/best_miou'.format(dataset_name), mean_iu, curr_iter)
...
def print_evaluate_results(hist, iu, dataset_name=None, dataset=None):
    ...
    for idx, i in enumerate(iu):
        ...
        fp = '{:5.1f}'.format(
            np.divide(iu_false_positive[idx], iu_true_positive[idx], out=np.zeros_like(iu_false_positive[idx]), where=iu_true_positive[idx]!=0))
        # fn = '{:5.1f}'.format(iu_false_negative[idx] / iu_true_positive[idx])
        fn = '{:5.1f}'.format(
            np.divide(iu_false_negative[idx], iu_true_positive[idx], out=np.zeros_like(iu_false_negative[idx]), where=iu_true_positive[idx]!=0)
            )
        # precision = '{:5.1f}'.format(
        #     iu_true_positive[idx] / (iu_true_positive[idx] + iu_false_positive[idx]))
        precision = '{:5.1f}'.format(
            np.divide(iu_true_positive[idx], (iu_true_positive[idx] + iu_false_positive[idx]), out=np.zeros_like(iu_true_positive[idx]),
            where=(iu_true_positive[idx] + iu_false_positive[idx])!=0)
            )
        # recall = '{:5.1f}'.format(
        #     iu_true_positive[idx] / (iu_true_positive[idx] + iu_false_negative[idx]))
        recall = '{:5.1f}'.format(
            np.divide(iu_true_positive[idx], (iu_true_positive[idx] + iu_false_negative[idx]), out=np.zeros_like(iu_true_positive[idx]),
            where=(iu_true_positive[idx] + iu_false_negative[idx])!=0)
            )
        logging.info('{}    {}   {}  {}     {}  {}   {}   {}'.format(
