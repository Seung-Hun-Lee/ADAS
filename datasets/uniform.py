...
def class_centroids_image(item, tile_size, num_classes, id2trainid):
    ...
    # import imageio
    # mask = imageio.imread(label_fn, format='PNG-FI')
...
def class_centroids_image_from_color(item, tile_size, num_classes, id2trainid):
    ...
    image_fn, label_fn = item
    centroids = defaultdict(list)
    mask = Image.open(label_fn)
    mask = np.asarray(mask, np.float32)
    # mask = np.asarray(imread(label_fn), np.uint8)
    # mask = np.array(imread(label_fn), dtype=np.float64) / 255
    image_size = mask.shape
    tile_locations = calc_tile_locations(tile_size, image_size)
    ...
    # if id2trainid:
    #     for k, v in id2trainid.items():
    #         # print("0", ((mask == np.array(k))[:,:,0] & (mask == np.array(k))[:,:,1] & (mask == np.array(k))[:,:,2]).shape)
    #         # print("0", mask.shape)
